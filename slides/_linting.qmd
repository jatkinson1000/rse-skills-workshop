...
- examine the report and try and address some of the issues.
  ...
  - For a challenge try and fix: `B904` try exceptions and `RUF059` unused unpack

### Astrophysics

Go to exercise 3 and:

- run `ruff check` on `astro.py`
- examine the report and try and address some of the issues.
  - Try and deal with: `F401` unused imports, `I001` unsorted imports,
    `B006` dangerous default, and `D202` Blank lines
  - For a challenge try and fix: `A001` shadowing, and `PLR1714` merging comparisons.
  - Ignore `F841` for now as we will address it in a later exercise.
...
- We will add the entire `"D"` (pydocstyle) ruleset, extending to `D417`.
