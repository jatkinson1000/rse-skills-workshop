...
Go to exercise 1 and examine the code in `geo.py`.

### Astrophysics

Scenario: you have just finished preparing a new stellar dataset in a csv file and want
to visualise the data to see what it tells you.

You know that your colleague has produced relevant figures and analysis for similar data
before, so ask them for a copy of their code (yay, reuse :+1:).

Go to exercise 1 and examine the code in `astro.py`
...
- list of required packages to be installed by pip
- typically specify top-level, not subdependencies
- can enforce version constraints
...
We can use `pip freeze` to get a list of all dependencies and versions, though this includes subdependencies.
:::
