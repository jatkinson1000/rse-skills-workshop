...
def filter(df, sgr):
    ...
    dataq = df[
        ...
        & (df.chi2 < -2.39) & (df.double == "no")  # Chi_sq limit of -2.39
        & ~(df['starname'].isin(sgr['starname']))
        & (df.b.abs() < 10.0) & ((df.b < 0) | (df.l < 0))
    ]
    return dataq
...
def hrfeh(dataq, highlogggrens, lowlogggrens, my_cmap):
    ...
    plot1 = ax1.scatter(dataq.Teff, dataq.logg, c=dataq.FeH, vmax=0.5, vmin=-2.8, cmap=cmap, s=5)
    cbar1 = plt.colorbar(plot1, cax=cax1, label=r"$\mathrm{[Fe/H]}$")
    ...
    ax2.hist(dataq.FeH,  bins=np.arange(-3.0, 0.501, 0.1), label=r"$\mathrm{all}$",
        histtype="step", linewidth=2.0,                    color="grey")
    ax2.legend(loc=1)
...
def compute_rv_gc(data):
    ...
    rv_gc = (
        data.rv+220 * np.sin(data.l*np.pi/180) * np.cos(data.b*np.pi/180)
        + 16.5* (np.sin(data.b*np.pi/180) * np.sin(25*np.pi/180)
        + np.cos(data.b * np.pi / 180)*np.cos(25 * np.pi / 180)*np.cos((data.l - 53) * np.pi / 180))  
    )
    data["rv_gc"] = rv_gc
    return data
...
def plot_velocity_stats(data, data2, highlogggrens, lowlogggrens, plot_labels=["typical", "older", "oldest"]):
    """
    ...
    for minfeh, maxfeh, kolom, description in zip(
        ...
        for gr in ["-", "+"]:
            ...
            for l in llist:
                ...
                vlen = vlens.append(len(b_l.rv_gc))
            ...
            nbin_mean2 = np.mean(nbin2)

            ax[kolom].scatter(np.array(ls),
                              v_avs,  
                              color="grey",  
                              zorder=9,  
                              marker="o",    
        ...
        FeHdat = data[
            (data.FeH <= maxfeh)         & (data.FeH >= minfeh)
            & (data.logg > lowlogggrens) & (data.logg < highlogggrens)
        ]
        ax[kolom].text(11, -90, r"$N_\mathrm{*} =\,$" + str(len(FeHdat)), fontsize=18)

        ax[kolom].set_title(
            "$"+str(minfeh)+r" < \mathrm{[Fe/H]} < "+str(maxfeh)+"$ \n ({})".format(description),
            fontsize=s2,
        )

        if (kolom == 1) or (kolom == 2) or (kolom == 3):
...
def brava():
    ...
    return pd.read_csv("../../data/brava-survey.dat", sep=r"\s+", names=["l", "rv_mean", "rv_sig"])
...
def main():
    ...
    plot_velocity_stats(data, vb_8, highlogggrens, lowlogggrens, plot_labels=["typical stars", "older stars", "the oldest stars"])
